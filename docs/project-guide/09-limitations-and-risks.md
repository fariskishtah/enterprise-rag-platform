...
| Workspace scoping is inconsistent. | Upload and Chat read `knowledgeBase`; `IntelligencePage.tsx:57-67` chooses the first returned KB. Settings ignores the query. Workspace “Evaluation” points to `/intelligence` (`WorkspacePage.tsx:19`). | Select the KB again on destination pages. Fix route/query use in a future product change. |
...
| Sidebar workspace switcher and recent items are presentation-only. | Switcher has no handler (`AppShell.tsx:144-151`); Recent labels are fixed links (`:180-190`). | Do not present them as dynamic account/workspace/recent data. |
| Dashboard retrieval health is decorative. | Five percentages are hard-coded (`DashboardPage.tsx:243-248`); grade is based only on any ready source. | Use evaluation runs and retrieval debug for real measurements. Label/remove decorative metrics in a future UI change. |
...
- There are 30 automated test files, but only eight frontend unit-test files. Several major pages rely mostly on Playwright/backend coverage.
...
The audit found no confirmed compile/runtime break in the inspected product paths. The strongest likely defect is the knowledge-base workspace's **Evaluation** shortcut pointing to `/intelligence` rather than `/evaluation`, plus destinations that ignore its intended scope (`frontend/src/pages/WorkspacePage.tsx:12-20`). The feedback, evaluation-creation, media-delete, demo-seed, workspace switcher, chat cancel, and dynamic-recent flows are better described as partial/missing UI rather than broken backend features.
